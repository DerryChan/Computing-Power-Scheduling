...
function renderNodes(nodes) {
  $('nodes').innerHTML = nodes.map(node => {
    ...
    return `<div class="node-card">
      ...
      <div class="node-meta">${esc(node.model)} · RTT ${node.rtt_ms} ms · 成本 ${(node.cost||0).toFixed(2)} · 可用 ${(node.free_gb||0).toFixed(1)} GB${node.agent_url? ' · '+esc(node.agent_url):''}${node.last_error? ' · '+esc(node.last_error):''}</div>
